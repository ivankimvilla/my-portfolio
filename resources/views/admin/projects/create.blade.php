...
<div class="max-w-3xl">
    ...
    <form method="POST" action="{{ route('admin.projects.store') }}" class="bg-gray-900 p-8 rounded-lg border border-gray-800 space-y-6">
        @csrf

        <div>
            <label class="block text-sm font-semibold mb-2">Project Title *</label>
            <input type="text" name="title" required class="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600 @error('title') ring-red-600 @enderror" value="{{ old('title') }}">
            @error('title')<p class="text-red-400 text-sm mt-1">{{ $message }}</p>@enderror
        </div>

        <div>
            <label class="block text-sm font-semibold mb-2">Slug *</label>
            <input type="text" name="slug" required class="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600 @error('slug') ring-red-600 @enderror" value="{{ old('slug') }}" placeholder="project-slug">
        ...
        <div>
            ...
            <textarea name="problem_solution" rows="6" class="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600">{{ old('problem_solution') }}</textarea>
        ...
        <div class="grid md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="url" name="image_url" class="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600" value="{{ old('image_url') }}" placeholder="https://example.com/image.jpg">
            </div>
        ...
        <div class="grid md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="url" name="live_url" class="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600" value="{{ old('live_url') }}">
            </div>

            <div>
                <label class="block text-sm font-semibold mb-2">GitHub URL</label>
                <input type="url" name="github_url" class="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600" value="{{ old('github_url') }}">
            </div>
        </div>

        <div>
            <label class="block text-sm font-semibold mb-2">Technologies (comma-separated)</label>
            <input type="text" name="technologies" class="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600" value="{{ old('technologies') }}" placeholder="Laravel, Vue.js, MySQL">
            <p class="text-xs text-gray-400 mt-1">e.g., Laravel, React, MySQL</p>
        </div>

        <div>
            <label class="block text-sm font-semibold mb-2">Your Role</label>
            <textarea name="role" rows="3" class="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600">{{ old('role') }}</textarea>
